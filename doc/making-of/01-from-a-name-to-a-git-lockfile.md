# From a name to a git-backed lockfile

*Part one of [diderot's making-of](../../MAKING-OF.md): the gap survey, the name, the
Go-vs-Java call, the Helm transposition, and milestone M1 — `update`/`install`/`status`
over git sources. Published on the blog as
[Building diderot, part 1](https://blog.sunix.org/posts/building-diderot-a-package-manager-for-ai-agent-skills-one-prompt-at-a-time-part-1/).*
