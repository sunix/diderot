...
7. [Starting the signing work: two walls before any feature](doc/making-of/07-signing-foundations.md) —
   resuming what part three parked: where a registry can even keep a signature, discovering that
   ghcr.io does not implement the OCI referrers API (measured against zot, since a bare 404 proves
   nothing) and that the spec had already written the fallback, seven native builds to get
